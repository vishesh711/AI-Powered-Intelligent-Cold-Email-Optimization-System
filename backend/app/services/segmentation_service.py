...
class SegmentationService:
    ...
    def segment_prospects(
        ...
        # Perform clustering
        if algorithm == "kmeans":
            clusters, cluster_centers = self._kmeans_clustering(scaled_features, n_clusters)
        elif algorithm == "dbscan":
            clusters, cluster_centers = self._dbscan_clustering(scaled_features, params)
            n_clusters = len(set(clusters)) - (1 if -1 in clusters else 0)
        elif algorithm == "hierarchical":
            clusters, cluster_centers = self._hierarchical_clustering(scaled_features, n_clusters)
        else:
            raise ValueError(f"Unsupported algorithm: {algorithm}")
        
        # Create visualization data (PCA for 2D visualization)
        visualization_data = self._create_visualization_data(scaled_features, clusters, cluster_centers)
        
        # Create segments
        segments = self._create_segments(prospect_data, clusters, feature_names, scaled_features, cluster_centers)
    ...
    def _extract_features(self, prospect_data: List[Dict[str, Any]]) -> tuple:
        ...
        if 'company_size' in df.columns:
            df['company_size_num'] = df['company_size'].map(lambda x: size_mapping.get(x, 0))
        ...
        if 'seniority' in df.columns:
            df['seniority_num'] = df['seniority'].map(lambda x: seniority_mapping.get(x, 0))
        ...
        for feature in categorical_features:
            ...
            tfidf_df = pd.DataFrame(
                tfidf.toarray(),
                columns=[f"{feature}_{term}" for term in vectorizer.get_feature_names_out()]
            )
    ...
    def _create_visualization_data(self, features, clusters, cluster_centers):
        ...
        for cluster_id in unique_clusters:
            ...
            if len(cluster_points) > 0:
                datasets.append({
                    "label": f"Segment {cluster_id + 1}" if cluster_id != -1 else "Outliers",
                    "data": [{"x": float(point[0]), "y": float(point[1])} for point in cluster_points],
                    "backgroundColor": f"rgba({(cluster_id * 50) % 256}, {(cluster_id * 100) % 256}, {(cluster_id * 150) % 256}, 0.5)" if cluster_id != -1 else "rgba(0, 0, 0, 0.3)"
                })
        
        return {
            "datasets": datasets,
            "centers": [{"x": float(center[0]), "y": float(center[1])} for center in pca_centers]
        }
    
    def _create_segments(self, prospect_data, clusters, feature_names, scaled_features, cluster_centers):
